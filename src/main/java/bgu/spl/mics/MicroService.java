...
public abstract class MicroService implements Runnable {
    ...
    private final ConcurrentHashMap<Class<? extends Message>,Callback<? extends Message>> messagesandActions;
    ...
    public MicroService(String name) {
        ...
        messagesandActions=new ConcurrentHashMap<>();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        //adding the appropriate callback if it is not there yet,
        messagesandActions.putIfAbsent(type,callback);
        //updating the event in the message bus that this micro can handle it.
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        //adding the appropriate callback to this broadcast.
        messagesandActions.putIfAbsent(type,callback);
        //updating the message bus there is someone that can handle it.
    ...
    public final void run() {
        //first we register the microservice.
        messageBus.register(this);
        //intialize it - subscribing to all the events and broadcasts needed
        initialize();
        while(loop) {
            try {
                //while we didn't finish the program each microservice will wait for it to get a message.
              Message  m = messageBus.awaitMessage(this);
              //once it gets a message (await is blocking) it will call the callback from the hashmap of the callbacks and excute it.
                Callback callback=messagesandActions.get(m.getClass());
                callback.call(m);
        ...
        //unregister the microservice from the bus.
